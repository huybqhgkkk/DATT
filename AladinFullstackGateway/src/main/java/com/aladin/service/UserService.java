...
import com.aladin.service.dto.UserDTO;
...
public class UserService {
    ...
    private final Logger log = LoggerFactory.getLogger(com.aladin.service.UserService.class);
    ...
    public UserService(UserRepository userRepository, UsergroupService usergroupService, AuthorityRepository authorityRepository) {
    ...
    public Mono<User> saveUser(User user) {
        log.info("saving an user:"+user);
        return saveUser(user, false);
    }

    @Transactional
    public Mono<User> saveUser(User user, boolean forceCreate) {
        log.info("saving an user:"+user + ",forceCreate: "+forceCreate);
        return SecurityUtils
            .getCurrentUserLogin()
            ...
            .flatMap(
                login -> {
                    ...
                    log.info("modify an user:"+user +",login:"+login);
                    Mono<User> persistedUser=userRepository.findOneByLogin(login)
                        .switchIfEmpty(userRepository.create(user))
                        .doOnNext(us -> userRepository.save(user));
                    persistedUser.block();
    ...
    private Mono<User> syncUserWithIdP(Map<String, Object> details, User user) {
        log.info("syncing:details:"+details + ", user:"+user);
        ...
        return getAuthorities()
            .collectList()
            .doOnNext(
                listString -> log.debug("listAuthorities:"+listString )
            )
            .flatMapMany(
                dbAuthorities -> {
                    List<Authority> authoritiesToSave = userAuthorities
                        ...
                        .map(
                            authority -> {
                                ...
                                log.info("authority:"+authority + ", authorityToSave:"+authorityToSave);
    ...
    private void setusergroup(Map<String, Object> details, User user) {
        String group = null;
        String isleader ="0";
        String roles = null;
        if (details.get("group") != null) {
           group = String.valueOf(details.get("group"));
           group = group.replaceAll("[^0-9a-zA-Z,]", "");

        }
        if (details.get("roles") != null) {
            roles = String.valueOf(details.get("roles"));
        ...
        usergroupService.update(group,isleader,user.getEmail(),user.getId());
        usergroupService.save(group,isleader,user.getEmail(),user.getId());
    }
}
