...
public class EmailtemplateResource {
    ...
    /**
     * {@code PATCH  /emailtemplates/:id} : Partial updates given fields of an existing emailtemplate, field will ignore if it is null
     *
     * @param id the id of the emailtemplate to save.
     * @param emailtemplate the emailtemplate to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated emailtemplate,
     * or with status {@code 400 (Bad Request)} if the emailtemplate is not valid,
     * or with status {@code 404 (Not Found)} if the emailtemplate is not found,
     * or with status {@code 500 (Internal Server Error)} if the emailtemplate couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/emailtemplates/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Emailtemplate> partialUpdateEmailtemplate(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Emailtemplate emailtemplate
    ) throws URISyntaxException {
        log.debug("REST request to partial update Emailtemplate partially : {}, {}", id, emailtemplate);
        if (emailtemplate.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, emailtemplate.getId())) {
        ...
        Optional<Emailtemplate> result = emailtemplateService.partialUpdate(emailtemplate);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, emailtemplate.getId().toString())
        );
    }
    ...
    /**
     * {@code DELETE  /emailtemplates/:id} : delete the "id" emailtemplate.
     *
