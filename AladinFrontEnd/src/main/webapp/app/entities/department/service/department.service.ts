...
import { getDepartmentIdentifier, IDepartment } from '../department.model';
...
export class DepartmentService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/departments', 'aladintechcobackendtest');
  protected resourceUrlPB = this.applicationConfigService.getEndpointFor('api/departmentsname', 'aladintechcobackendtest');
  ...
  findPB(name: string): Observable<EntityArrayResponseType> {
    return this.http.get<IDepartment[]>(`${this.resourceUrlPB}/${name}`, { observe: 'response' });
  }
  ...
  queryPB(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IDepartment[]>(this.resourceUrlPB, { params: options, observe: 'response' });
  }
